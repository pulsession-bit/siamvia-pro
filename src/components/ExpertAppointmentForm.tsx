...
const ExpertAppointmentForm: React.FC<ExpertAppointmentFormProps> = ({
  ...
  const contactMethods = [
    { id: 'whatsapp', icon: MessageSquare, label: 'WhatsApp', color: 'text-green-500' },
    { id: 'phone', icon: Phone, label: 'Appel', color: 'text-blue-500' },
    { id: 'email', icon: Mail, label: 'Email', color: 'text-amber-500' },
    { id: 'video', icon: Video, label: 'Vidéo', color: 'text-purple-500' }
  ];
  ...
  return (
    <form onSubmit={actions.submitForm} className={`w-full space-y-8 transition-all duration-300 ${state.isSubmitting ? 'opacity-50 pointer-events-none grayscale' : 'opacity-100'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label className="flex items-center text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">
            <Calendar className="w-3 h-3 mr-2" /> Date souhaitée
          </label>
          <input
            type="date"
        ...
        <div className="space-y-2">
          <label className="flex items-center text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">
            <Clock className="w-3 h-3 mr-2" /> Heure
          </label>
          <input
            type="time"
            className="w-full rounded-2xl border border-slate-100 bg-slate-50 px-5 py-4 focus:bg-white focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 outline-none transition-all font-bold text-slate-700"
      ...
      <div className="space-y-4">
        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Canal de contact préféré</label>
        ...
        <div className="relative group mt-6">
          <input
            ...
            placeholder={state.contactMethod === 'email' ? 'votre@email.com' : 'N° avec indicatif (ex: +33...)'}
            required
          />
